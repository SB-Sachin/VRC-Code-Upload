...
/*    Description:  V5 Competition Project - Combined Auton & Driver          */
/*                  with Autonomous Selector                                  */
/*                                                                            */
/*----------------------------------------------------------------------------*/
#include "vex.h"
...
void drive_forward(int distance, int intakeSpeed = 0) {
  ...
  stopAllMotors();
  intake_bottom.stop(brake); // Stop intake motor after driving
}
...
void turn(double targetHeading, int intakeSpeed = 0) {
  ...
  stopAllMotors();
  intake_bottom.stop(brake); // Stop intake motor after turning
}
...
void split_Drive() {
    double forward = cont.Axis3.position() * DRIVER_SPEED;
    double turn = cont.Axis1.position() * TURN_SPEED;

    //    double rightSpeed = ((((cont.Axis3.position()/100)^3)*100) + (((cont.Axis1.position()/100)^3)*100)) * DRIVE_SPEED;
    //double leftSpeed = ((((cont.Axis3.position()/100)^3)*100) - (((cont.Axis1.position()/100)^3)*100)) * DRIVER_SPEED;
    ...
    bool currentButtonBState = cont.ButtonA.pressing();
    ...
    if(cont.ButtonL1.pressing()) {
        intake_bottom.spin(vex::forward, 100, pct);
...
void brainDisplay() {
  while (true) {
    ...
    if (autonSelection == 0) {
      Brain.Screen.printAt(1, 20, "Auton: RIGHT SIDE");
    } else {
      Brain.Screen.printAt(1, 20, "Auton: LEFT SIDE");
    }
...
void displayAutonSelector() {
  ...
  // Draw title
  Brain.Screen.printAt(160, 20, "SELECT AUTONOMOUS");
  ...
  Brain.Screen.drawRectangle(20, 60, 200, 80);
  Brain.Screen.setPenColor(color::white);
  Brain.Screen.printAt(70, 110, "RIGHT SIDE");
  ...
  Brain.Screen.drawRectangle(260, 60, 200, 80);
  Brain.Screen.setPenColor(color::white);
  Brain.Screen.printAt(315, 110, "LEFT SIDE");
  
  Brain.Screen.setFillColor(color::black);
  Brain.Screen.setPenColor(color::white);
  Brain.Screen.printAt(140, 180, "Tap to select autonomous");
}
...
void autonSelector() {
  ...
  while (true) {
    if (Brain.Screen.pressing()) {
      ...
      // Check if RIGHT SIDE button pressed
      if (x >= 20 && x <= 220 && y >= 60 && y <= 140) {
        autonSelection = 0;
        displayAutonSelector();
        wait(200, msec); // Debounce
      }
      // Check if LEFT SIDE button pressed
      else if (x >= 260 && x <= 460 && y >= 60 && y <= 140) {
        autonSelection = 1;
        displayAutonSelector();
        wait(200, msec); // Debounce
      }
...
void auton_left_side() {
  // LEFT SIDE autonomous routine (mirrored)
  gyro_1.setHeading(0, degrees);
...
void pre_auton(void) {
  ...
  thread selectorThread(autonSelector);
